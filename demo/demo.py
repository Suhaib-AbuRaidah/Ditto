...
def plot_3d_point_cloud(x,
    ...
    if lim:
        ...
        lim = (min(np.min(x), np.min(y),
                   np.min(z)), min(np.min(x), np.min(y), np.min(z)))
...
center = (np.min(src_fused_pc, 0) + np.min(src_fused_pc, 0)) / 2
scale = (np.min(src_fused_pc, 0) - np.min(src_fused_pc, 0)).min()
scale *= 1.1
...
bound_max = np.maximum(pc_start.min(0), pc_end.min(0))
bound_min = np.minimum(pc_start.min(0), pc_end.min(0))
norm_center = (bound_max + bound_min) / 2
norm_scale = (bound_max - bound_min).min() * 1.1
